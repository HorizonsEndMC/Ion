...
open class CustomBlock(
	...
	fun getDrops(itemUsed: ItemStack? = null): Array<ItemStack> {
		...
		val isSpecialItem = customItem != null && customItem.id.lowercase(Locale.getDefault()).replace("drill", "pickaxe").contains(tool)
...
object CustomBlocks {
	...
	private val blockDataMap = Object2ObjectOpenHashMap<BlockState, CustomBlock>()
	...
	val MINERAL_CHETHERITE = mineral("chetherite", setOf(EAST, NORTH, UP), setOf(SOUTH, UP), "chetherite", 1)
	...
	operator fun get(blockData: BlockState): CustomBlock? = blockDataMap[blockData]
}
