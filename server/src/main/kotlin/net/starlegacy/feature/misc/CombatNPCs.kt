...
object CombatNPCs : SLComponent() {
	...
	override fun onEnable() {
		listen<PlayerJoinEvent>(EventPriority.LOWEST, false, { event -> KilledPlayers.onJoin(event.player) })
		...
		listen<EntityDamageEvent> { event ->
			...
			val helmet = (entity as? ArmorStand)?.helmet ?: return@listen
			helmetMap[entity.uniqueId] = helmet
		}

		listen<EntityDeathEvent>(EventPriority.LOWEST) { event ->
			val entity: ArmorStand = event.entity as? ArmorStand ?: return@listen
