...
object BlueprintCommand : net.horizonsend.ion.server.command.SLCommand() {
	...
	fun onSave(sender: Player, name: String, @Optional confirm: String?) = asyncCommand(sender) {
		...
		if (createNew) {
			...
			Blueprint.create(slPlayerId, name, starship.data.starshipType.actual, pilotLoc, starship.initialBlockCount, data)
	...
	fun onList(sender: Player) = asyncCommand(sender) {
		...
		MenuHelper.apply {
			val items: List<GuiItem> = blueprints.map { blueprint ->
				guiButton(blueprint.type.actual.menuItem) {
	...
	fun onLoad(sender: Player, name: String) = asyncCommand(sender) {
		...
		Tasks.syncBlocking {
			...
			loadSchematic(sender, schematic, Vec3i(pilotLoc)) { origin ->
				tryPilot(sender, origin, blueprint.type.actual, blueprint.name)
			}
		}
	}
	...
	fun onFix(sender: Player, name: String) = asyncCommand(sender) {
		...
		Tasks.syncBlocking {
			...
			loadSchematic(sender, schematic, Vec3i(pilotLoc)) { origin ->
				tryPilot(sender, origin, blueprint.type.actual, blueprint.name) { starship ->
