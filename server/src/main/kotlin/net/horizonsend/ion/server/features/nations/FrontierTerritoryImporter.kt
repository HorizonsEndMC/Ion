...
object FrontierTerritoryImporter {
	private data class UpdateData(val id: Oid<FrontierTerritory>, val polygonData: ByteArray)
...
//                val territoryId = key.trim().replace("\n", "")
				...
				val existing = FrontierTerritory.findByName(label)
				if (existing != null) {
		...
		for ((existingId, polygonData) in update) {
			FrontierTerritory.setPolygonData(existingId, polygonData)
		}

		for ((label, worldName, polygonData) in import) {
			FrontierTerritory.create(label, worldName, polygonData)
		}
