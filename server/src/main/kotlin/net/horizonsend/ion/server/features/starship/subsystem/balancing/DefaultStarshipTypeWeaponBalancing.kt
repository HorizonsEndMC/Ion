...
class DefaultStarshipTypeWeaponBalancing(private val type: StarshipType) : StarshipWeaponBalancingManager() {
	...
	private val commandBursts = ConfigurationFiles.starshipBalancing().commandBurstDefaults.commandBursts
		.associateByTo(mutableMapOf()) { it.clazz }
		.apply { putAll(type.balancing.commandBurstOverrides.associateBy { it.clazz }) }
