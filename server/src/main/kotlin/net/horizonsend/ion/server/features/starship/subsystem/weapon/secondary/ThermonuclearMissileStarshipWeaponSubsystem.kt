...
import net.horizonsend.ion.server.configuration.starship.ThermonuclearMissileBalancing
import net.horizonsend.ion.server.core.registration.keys.CustomItemKeys
import net.horizonsend.ion.server.features.multiblock.type.starship.weapon.heavy.ThermonuclearMissileStarshipWeaponMultiblock
...
class ThermonuclearMissileStarshipWeaponSubsystem(
    starship: ActiveStarship,
    pos: Vec3i,
    override var face: BlockFace,
    private val multiblock: ThermonuclearMissileStarshipWeaponMultiblock,
    private val upOrDown: BlockFace
) : BalancedWeaponSubsystem<ThermonuclearMissileBalancing>(starship, pos, starship.balancingManager.getWeaponSupplier(ThermonuclearMissileStarshipWeaponSubsystem::class)), HeavyWeaponSubsystem, ManualWeaponSubsystem, DirectionalSubsystem, AmmoConsumingWeaponSubsystem {
	...
	override fun manualFire(shooter: Damager, dir: Vector, target: Vector) {
		val initialLaunchDirection = when (multiblock) {
			is TopThermonuclearMissileWeaponMultiblock -> BlockFace.UP.direction
			is BottomThermonuclearMissileWeaponMultiblock -> BlockFace.DOWN.direction
			else -> face.direction
		}
		...
		for (newBoid in 0 until 1) {
			Tasks.syncDelay(newBoid.toLong()) {
				...
				ThermonuclearMissileProjectile(
	...
	override fun getName(): Component {
		return Component.text("Thermonuclear Missile")
	}
