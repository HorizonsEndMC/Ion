...
abstract class ResultHolder<E: RecipeEnvironment, R: RecipeResult<E>>(val result: R) {
	fun verifySpace(environment: E) = result.verifySpace(environment)

	/**
	 * Builds the result of the recipe in the result environment. Do not directly set the results in this function, it may result in duplications if errors arise elsewhere
	 **/
	abstract fun buildTransaction(input: E, resultEnvironment: ResultExecutionEnvironment<E>)

	private val callbacks = mutableListOf<(E, RecipeExecutionResult?) -> Unit>()

	fun executeCallbacks(environment: E, result: RecipeExecutionResult?) {
		callbacks.forEach { t -> t.invoke(environment, result) }
	}
	...
	fun playSound(sound: Sound, requireSuccess: Boolean): ResultHolder<E, R> {
		callbacks.add { environment: E, result: RecipeExecutionResult? ->
			if (requireSuccess && result is RecipeExecutionResult.SuccessExecutionResult) environment.playSound(sound)
		}
	...
	fun updateFurnace(): ResultHolder<E, R> {
		callbacks.add { environment: E, executionResult: RecipeExecutionResult? ->
			val furnace = environment.multiblock.getInventory(0, 0, 0)?.holder as? Furnace ?: return@add
			...
			} else if (environment.multiblock is TickedMultiblockEntityParent) {
				furnace.burnTime = (environment.multiblock as TickedMultiblockEntityParent).tickingManager.interval.toShort()
			}
	...
	fun updateProgressText(): ResultHolder<E, R> {
		callbacks.add { environment: E, result: RecipeExecutionResult? ->
			...
			val entity = environment.multiblock
	...
	companion object {
		fun <T: ItemResultEnvironment> of(result: ItemResult<T>) = ItemResultHolder(result)
		//TODO more results
	}

	class ItemResultHolder<E: ItemResultEnvironment, R: ItemResult<E>>(result: R) : ResultHolder<E, R>(result) {
		override fun buildTransaction(input: E, resultEnvironment: ResultExecutionEnvironment<E>) {
			result.buildTransaction(input, resultEnvironment)
		}
	}
}
