...
fun getNeighborNodes(position: Long, nodes: Map<Long, TransportNode>, checkFaces: Set<BlockFace> = ADJACENT_BLOCK_FACES) = checkFaces.associateWithNotNull {
	val x = getX(position)
	val y = getY(position)
	val z = getZ(position)
...
fun getNeighborNodes(position: Long, nodes: Collection<Long>, checkFaces: Set<BlockFace> = ADJACENT_BLOCK_FACES) = checkFaces.mapNotNull {
	val x = getX(position)
	val y = getY(position)
	val z = getZ(position)
