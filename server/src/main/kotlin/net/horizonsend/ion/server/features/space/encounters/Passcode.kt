...
object Passcode : Encounter(identifier = "passcode") {
	...
	fun playerFails(player: Player) {
		...
		player.playSound(player, Sound.BLOCK_NOTE_BLOCK_BASS, 5.0f, 1.0f)
	}

	fun playerSucceeds(player: Player, chest: Chest) {
		player.closeInventory()
		player.success("You entered the correct passcode! The chest is now unlocked.")
		chest.location.world.playSound(chest.location, Sound.BLOCK_NOTE_BLOCK_HARP, 5.0f, 2.0f)
		Encounters.setChestFlag(chest, NamespacedKeys.INACTIVE, "true")
	}

	fun playerContinues(chest: Chest) {
		chest.location.world.playSound(chest.location, Sound.BLOCK_NOTE_BLOCK_SNARE, 5.0f, 1.0f)
	}
	...
	override fun generate(world: World, chestX: Int, chestY: Int, chestZ: Int) {
		...
		Encounters.setChestFlag(
			world.getBlockAt(chestX, chestY, chestZ).state as Chest,
			NamespacedKeys.PASSCODE_CODE,
			passcode
		)
	}
	...
	override fun onChestInteract(event: PlayerInteractEvent) {
		...
		val passcode = Encounters.getChestFlag(chest, NamespacedKeys.PASSCODE_CODE)
		var currentCode = ""
