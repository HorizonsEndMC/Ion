...
class MiscListeners : SLEventListener() {
	...
	fun onPlayerQuitEvent(event: PlayerQuitEvent) {
		...
		for (otherPlayer in player.world.players) {
			...
			if (distance <= 1_000) {
				log.warn("\"${player.name}\" has potentially combat logged. \"${otherPlayer.name}\" is ${distance}m away.")
			}
		}
	}
	...
	fun onEntityDamage(event: EntityDamageEvent) {
		...
		if (event.cause == EntityDamageEvent.DamageCause.DROWNING && isWearingSpaceSuit(player)) {
