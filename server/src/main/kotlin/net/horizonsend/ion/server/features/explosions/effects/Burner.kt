...
import net.horizonsend.ion.server.features.explosions.utilities.BlockPalette
import net.horizonsend.ion.server.features.explosions.utilities.BurnPalette
import net.horizonsend.ion.server.features.explosions.utilities.randomDirection
...
class Burner(val options: BurnOptions): Closeable {
    ...
    fun burn(block: Block, renderLocation: Location, heat: Float) {
        ...
        if (block.type.isAir && canPlaceFireOn(blockBelow)) {
            ...
            Tasks.asyncDelay(20 * 10) {
                if (isClosed) return@asyncDelay
        ...
        for ((delay, data) in burnPalette) {
			Tasks.asyncDelay(delay){
				if (isClosed) return@asyncDelay
				if (block.type.isAir)  return@asyncDelay
				//setBlock(block, data)
			}
        }
