...
import net.horizonsend.ion.server.features.starship.control.signs.display.toVector
...
class BookmarkDisplayFeature(
	...
	override fun tick() {
		...
		val offset = when(display.state){
			MapState.LOCAL_MAP ->(display.ship.centerOfMass.toVector().add(bookmark.toVector().multiply(-1))).setY(0).multiply(1.0 / display.maxDistance)
		...
		if(offset.x.absoluteValue > .5  || offset.z.absoluteValue > .5){
			...
			display.bookmarkTracked.remove(bookmark)
			this.despawn()
		}
