...
class NodeCacheFactory private constructor(private val materialHandlers: Map<Material, MaterialHandler<*>>) {
	...
	class Builder	{
		...
		inline fun <reified T: BlockData> addDataHandler(material: Material, noinline constructor: (T) -> CachedNode): Builder {
			this.materialHandlers[material] = MaterialHandler<T>(T::class, material, constructor)
			return this
		}

		fun addSimpleNode(material: Material, node: CachedNode): Builder {
			this.materialHandlers[material] = MaterialHandler(BlockData::class, material) { node }
			return this
		}
	...
	class MaterialHandler<T: BlockData>(val blockDataClass: KClass<T>, val registeredMaterial: Material, val constructor: (T) -> CachedNode) {
		fun construct(blockData: BlockData): CachedNode? {
			if (blockDataClass.isInstance(blockData)) return null
