...
class BargeSink(starship: ActiveStarship) : StandardSinkProvider(starship) {
    val standard = StandardSinkProvider(starship)

    override fun setup() {
        starship.iterateBlocks { x, y, z ->
            if (starship.world.getBlockAt(x, y, z).customBlock == CustomBlocks.BARGE_REACTOR_CORE) {
        ...
        for (nearbyPlayer in starship.world.getNearbyPlayers(starship.centerOfMass.toLocation(starship.world), 69420.0)) {
            nearbyPlayer.playSound(Sound.sound(Key.key("horizonsend:starship.explosion.battlecruiser"), Sound.Source.AMBIENT, 7f, 1.0f))
        }
    ...
    private val reactor = starship.subsystems.filterIsInstance<BargeReactorSubsystem>().firstOrNull()
