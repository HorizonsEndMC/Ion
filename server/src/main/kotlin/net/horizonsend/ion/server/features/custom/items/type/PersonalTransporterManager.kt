...
object PersonalTransporterManager : SLEventListener() {
    ...
    fun checkItemFromPlayer(target: Player): Boolean {
        return target.inventory.contains(PERSONAL_TRANSPORTER.getValue().constructItemStack())
    }

    fun removeItemFromPlayer(target: Player) {
        if (checkItemFromPlayer(target)) {
            target.inventory.removeItemAnySlot(PERSONAL_TRANSPORTER.getValue().constructItemStack())
        }
    }
